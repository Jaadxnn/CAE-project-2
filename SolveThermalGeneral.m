...
%   - Insulated         (natural BC - nothing to add)
%   - Convection        (Robin BC)
%   - Uniform heat source Q_uniform [W/m^3]
%
% kx, ky     -> conductivity in x and y directions
% h          -> convection coefficient
% T_ambient  -> free stream temperature for convection
% =========================================================
...
% =========================================================
% 1. ASSEMBLE GLOBAL STIFFNESS AND HEAT SOURCE
% =========================================================
for e = 1:n_elements
    n1 = ncon(e,1);
    n2 = ncon(e,2);
    ...
    % Uniform heat source contribution — distributed evenly to 3 nodes
...
% =========================================================
% 2. CONVECTION BC ASSEMBLY (Robin)
%
% For each boundary edge between consecutive convection
% nodes, add:
%   Kc_edge = (h*L*t/6) * [2 1; 1 2]
%   Qc_edge = (h*L*t*T_ambient/2) * [1; 1]
% =========================================================
%{
if ~isempty(convectionNodes)
    convectionNodes = sort(convectionNodes(:));
    for i = 1:length(convectionNodes)-1
        ni = convectionNodes(i);
        nj = convectionNodes(i+1);

        % Only add edge if these are adjacent boundary nodes
        xi = X(ni); yi = Y(ni);
        xj = X(nj); yj = Y(nj);
...
if ~isempty(convectionNodes) && h > 0
    ...
    for i = 1:length(convectionNodes)-1
        ...
        % Skip if these two nodes are not actually adjacent
        % i.e. their distance is more than 1.5x the expected edge length
...
% =========================================================
% 3. APPLY FIXED TEMPERATURE BCs (Dirichlet)
% =========================================================
...
for i = 1:length(fixedNodes)
    ...
    % Modify RHS for non-zero Dirichlet
    Q = Q - Kt(:, node) * Tval;
